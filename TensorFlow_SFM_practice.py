...
def step(prev_output, x):
    ...
    state_forget_gate = tf.sigmoid(tf.add(
        tf.add(tf.multiply(W_state, tf.reduce_sum(tf.multiply(W_pre_state, x), 1, keep_dims=True)),
               tf.multiply(V_state, z_)), b_state), name='State_Forget')
    frequency_forget_gate = tf.sigmoid(tf.add(
        tf.add(tf.multiply(W_freq, tf.reduce_sum(tf.multiply(W_pre_freq, x), 1, keep_dims=True)),
               tf.multiply(V_freq, z_)), b_freq), name='Freq_Forget')

    combined_forget_gate = tf.multiply(state_forget_gate, frequency_forget_gate)

    input_gate = tf.sigmoid(tf.add(
        tf.add(tf.multiply(W_g, tf.reduce_sum(tf.multiply(W_pre_g, x), 1, keep_dims=True)), tf.multiply(V_g, z_)), b_g),
                            name='Input_Gate')

    modulation_gate = tf.tanh(tf.add(
        tf.add(tf.multiply(W_i, tf.reduce_sum(tf.multiply(W_pre_i, x), 1, keep_dims=True)), tf.multiply(V_i, z_)), b_i),
                              name='Modulation_Gate')

    omega = tf.multiply(tf.add(tf.multiply(W_omega, tf.reduce_sum(tf.multiply(W_pre_omega, x), 1, keep_dims=True)),
                               tf.multiply(V_omega, z_)), b_omega)
    ...
    #output has dimensions (60, 4, 60, 60)
...
with tf.Session() as sess:
   ...
   #X = np.random.rand(5, 5, 3).astype(np.float32)
    ...
    for i in X:
        #i has shape (60,23)
        output = sess.run(tf.scan(step,elems=i,initializer=initial_weights))